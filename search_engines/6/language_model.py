...
# Вычисление весов языковой модели
...
ordered_docs = sorted(
    ordered_docs, key=lambda d: docs[d].get('p', 0), reverse=True)
...
idcg_ordered_docs = sorted(
    ordered_docs, key=lambda d: docs[d].get('number', 0), reverse=True)
...
for i in range(len(idcg_ordered_docs)):
    if i == 0:
        ...
        idcg += float(docs[idcg_ordered_docs[i]]['number']) / np.log2(i + 2)
