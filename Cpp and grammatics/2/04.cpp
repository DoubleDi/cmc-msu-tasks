...
int main (int argc, char *argv[])
{
    ...
    complex center {argv[1]}, z, res;
    sscanf (argv[2], "%lg", &radius);
    sscanf (argv[3], "%d", &parts_num);

    const int command_num = argc-4;
    const int command_len = 20;
    char** commands = new char* [command_num+1];
    double angle = 2*M_PI/parts_num;

    // cout << "magic angle:" << angle << endl;

    for (int i = 0; i < command_num; i++) {
        commands[i] = new char [command_len];
        strcpy (commands[i], argv[i+4]);
    }

    commands[command_num] = nullptr;

    complex prev_z{radius, 0.00}, curr_z{};
    prev_z = prev_z + center;

    for (int i = 0; i < parts_num; i++) {
        curr_z = complex{radius*cos(angle*(i+1)), radius*sin(angle*(i+1))};
        curr_z = curr_z + center;
